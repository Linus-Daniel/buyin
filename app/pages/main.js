...
function Main() {
  return (
    ...
    <div className='font-poppins justify-center p-0 flex flex-wrap' > {data.map((product)=><ProductCard key={name} image={product.image} price={product.price} name={product.name} />)}</div>
    </main>
  )
}
